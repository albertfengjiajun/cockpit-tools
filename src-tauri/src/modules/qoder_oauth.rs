...
mod ipc {
    ...
    impl QoderIpcClient {
        pub fn connect(socket_path: &str) -> Result<Self, String> {
            ...
            std::thread::spawn(move || {
                let result = UnixStream::connect(socket_path_owned);
                let _ = tx.send(result);
            });

            let stream = rx
                .recv_timeout(Duration::from_secs(5))
                .map_err(|_| "连接 Qoder IPC 超时".to_string())?
                .map_err(|e| format!("连接 Qoder IPC 失败: {}", e))?;
            stream
                .set_read_timeout(Some(Duration::from_secs(5)))
                .map_err(|e| format!("设置 Qoder IPC 读取超时失败: {}", e))?;
            stream
                .set_write_timeout(Some(Duration::from_secs(5)))
                .map_err(|e| format!("设置 Qoder IPC 写入超时失败: {}", e))?;
        ...
        pub fn send_request(&mut self, method: &str, params: Value) -> Result<Value, String> {
            ...
            loop {
                if started.elapsed() > Duration::from_secs(12) {
                    return Err(format!("Qoder IPC 调用超时 ({}): 12s", method));
                }
                let message = self.read_message()?;
                let message_id = message.get("id").and_then(|value| value.as_u64());
                if message_id != Some(request_id) {
                    continue;
                }
                ...
                if let Some(error) = message.get("error") {
                    if !error.is_null() {
                        let msg = error
                            ...
                            .unwrap_or("未知错误");
                        return Err(format!("Qoder IPC 调用失败 ({}): {}", method, msg));
                    }
                }
        ...
        fn write_message(&mut self, value: &Value) -> Result<(), String> {
            let body = serde_json::to_vec(value)
                .map_err(|e| format!("序列化 Qoder IPC 消息失败: {}", e))?;
            let header = format!("Content-Length: {}\r\n\r\n", body.len());
            self.stream
                .write_all(header.as_bytes())
                .map_err(|e| format!("写入 Qoder IPC Header 失败: {}", e))?;
            self.stream
                .write_all(&body)
                .map_err(|e| format!("写入 Qoder IPC Body 失败: {}", e))?;
            self.stream
                .flush()
                .map_err(|e| format!("刷新 Qoder IPC 写入缓冲失败: {}", e))?;
            Ok(())
        }

        fn read_message(&mut self) -> Result<Value, String> {
            loop {
                if let Some(parsed) = try_take_message(&mut self.read_buffer)? {
                    return Ok(parsed);
                }
                ...
                let read = self
                    .stream
                    .read(&mut chunk)
                    .map_err(|e| format!("读取 Qoder IPC 消息失败: {}", e))?;
                if read == 0 {
                    return Err("Qoder IPC 连接已关闭".to_string());
                }
    ...
    fn try_take_message(buffer: &mut Vec<u8>) -> Result<Option<Value>, String> {
        let Some(header_end) = find_header_end(buffer.as_slice()) else {
            return Ok(None);
        };
        let header_text = std::str::from_utf8(&buffer[..header_end])
            .map_err(|e| format!("解析 Qoder IPC Header 失败: {}", e))?;

        let content_length = parse_content_length(header_text)
            .ok_or_else(|| "Qoder IPC 消息缺少 Content-Length".to_string())?;
        ...
        let value = serde_json::from_slice::<Value>(&body)
            .map_err(|e| format!("解析 Qoder IPC JSON 消息失败: {}", e))?;
        Ok(Some(value))
    }
...
fn read_qoder_cosy_info() -> Result<QoderCosyInfo, String> {
    ...
    if info_json_path.exists() {
        logger::log_info(&format!(
            "[Qoder OAuth] 读取 Qoder 运行信息文件: {}",
            info_json_path.to_string_lossy()
        ));
        let content = fs::read_to_string(&info_json_path)
            .map_err(|e| format!("读取 Qoder .info.json 失败: {}", e))?;
        let mut info = serde_json::from_str::<QoderCosyInfo>(&content)
            .map_err(|e| format!("解析 Qoder .info.json 失败: {}", e))?;
        if info.socket_path.is_none() {
            info.socket_path = info.ipc_server_path.clone();
        }
        logger::log_info(&format!(
            "[Qoder OAuth] Qoder 运行信息已加载: websocket_port={:?}, socket_path={}",
            info.websocket_port,
            info.socket_path.as_deref().unwrap_or("<none>")
        ));
        return Ok(info);
    }

    let info_path = shared_cache_dir.join(".info");
    if !info_path.exists() {
        return Err(format!(
            "未找到 Qoder 运行信息文件: {} 或 {}",
            info_json_path.to_string_lossy(),
    ...
    let content =
        fs::read_to_string(&info_path).map_err(|e| format!("读取 Qoder .info 失败: {}", e))?;
    let mut lines = content.lines();
    let websocket_port = lines
        .next()
        .and_then(|line| line.trim().parse::<u16>().ok());
    let _pid = lines.next();
    let socket_path = lines
        .next()
        .and_then(|line| normalize_non_empty(Some(line)));

    let info = QoderCosyInfo {
        websocket_port,
        socket_path,
        ipc_server_path: None,
    };
    logger::log_info(&format!(
        "[Qoder OAuth] Qoder 运行信息已加载(.info): websocket_port={:?}, socket_path={}",
        info.websocket_port,
        info.socket_path.as_deref().unwrap_or("<none>")
    ));
    Ok(info)
...
fn call_qoder_rpc(method: &str, params: Value) -> Result<Value, String> {
    ...
    let socket_path = info
        ...
        .ok_or_else(|| {
            format!(
                "Qoder IPC Socket 路径为空，请先启动 Qoder（websocketPort={:?}）",
                info.websocket_port
            )
        })?;

    logger::log_info(&format!(
        "[Qoder OAuth] IPC 调用开始: method={}, socket_path={}",
        method, socket_path
    ));
    let mut client = ipc::QoderIpcClient::connect(&socket_path)?;
    client.initialize()?;
    let result = client.send_request(method, params);
    match &result {
        Ok(value) => logger::log_info(&format!(
            "[Qoder OAuth] IPC 调用成功: method={}, elapsed={}ms, result_kind={}",
            method,
            started.elapsed().as_millis(),
            value_kind(value)
        )),
        Err(err) => logger::log_warn(&format!(
            "[Qoder OAuth] IPC 调用失败: method={}, elapsed={}ms, error={}",
            method,
            started.elapsed().as_millis(),
            err
        )),
...
async fn send_qoder_ws_request(
    ...
    *next_id += 1;
    ...
    let text = serde_json::to_string(&payload)
        .map_err(|e| format!("序列化 Qoder WebSocket 请求失败: {}", e))?;
    ws_stream
        .send(Message::Text(text.into()))
        .await
        .map_err(|e| format!("发送 Qoder WebSocket 请求失败: {}", e))?;

    loop {
        let message = tokio::time::timeout(Duration::from_secs(12), ws_stream.next())
            .await
            .map_err(|_| format!("Qoder WebSocket 调用超时 ({}): 12s", method))?
            .ok_or_else(|| "Qoder WebSocket 连接已关闭".to_string())?
            .map_err(|e| format!("读取 Qoder WebSocket 消息失败: {}", e))?;

        let parsed = match message {
            Message::Text(text) => serde_json::from_str::<Value>(text.as_ref())
                .map_err(|e| format!("解析 Qoder WebSocket JSON 消息失败: {}", e))?,
            Message::Binary(data) => serde_json::from_slice::<Value>(&data)
                .map_err(|e| format!("解析 Qoder WebSocket 二进制消息失败: {}", e))?,
            Message::Ping(data) => {
                ws_stream
                    .send(Message::Pong(data))
                    .await
                    .map_err(|e| format!("回复 Qoder WebSocket Ping 失败: {}", e))?;
                continue;
            }
            ...
            Message::Close(frame) => {
                let reason = frame
                    ...
                    .unwrap_or_else(|| "连接关闭".to_string());
                return Err(format!("Qoder WebSocket 连接已关闭: {}", reason));
            }
            _ => continue,
        };

        if parsed.get("id").and_then(|value| value.as_u64()) != Some(request_id) {
            continue;
        }
        ...
        if let Some(error) = parsed.get("error") {
            if !error.is_null() {
                let msg = error
                    ...
                    .and_then(|value| value.as_str())
                    .unwrap_or("未知错误");
                return Err(format!("Qoder WebSocket 调用失败 ({}): {}", method, msg));
            }
        }
...
async fn send_qoder_ws_notification(
    ...
    let text = serde_json::to_string(&payload)
        .map_err(|e| format!("序列化 Qoder WebSocket 通知失败: {}", e))?;
    ws_stream
        .send(Message::Text(text.into()))
        .await
        .map_err(|e| format!("发送 Qoder WebSocket 通知失败: {}", e))?;
    Ok(())
}
...
async fn call_qoder_rpc_async(method: String, params: Value) -> Result<Value, String> {
    let info = read_qoder_cosy_info()?;
    let websocket_port = info.websocket_port.ok_or_else(|| {
        format!(
            "Qoder WebSocket 端口为空，请先启动 Qoder（socketPath={}）",
            info.socket_path.as_deref().unwrap_or("<none>")
        )
    })?;
    let ws_url = format!("ws://127.0.0.1:{}/", websocket_port);

    logger::log_info(&format!(
        "[Qoder OAuth] WebSocket 调用开始: method={}, websocket_port={}, url={}",
        method, websocket_port, ws_url
    ));
    ...
    .await
    .map_err(|_| format!("连接 Qoder WebSocket 超时: {}", ws_url))?
    .map_err(|e| format!("连接 Qoder WebSocket 失败: {}", e))?;

    let mut next_id = 1u64;
    let _ = send_qoder_ws_request(
        &mut ws_stream,
        &mut next_id,
        "initialize",
        json!({
    ...
    .await?;
    send_qoder_ws_notification(&mut ws_stream, "initialized", json!({})).await?;
...
fn call_qoder_rpc(method: &str, params: Value) -> Result<Value, String> {
    ...
    let result = if let Ok(handle) = tokio::runtime::Handle::try_current() {
        std::thread::spawn(move || handle.block_on(call_qoder_rpc_async(method_name, params)))
            .join()
            .map_err(|_| "Qoder WebSocket 调用线程 panic".to_string())?
    } else {
        let runtime = tokio::runtime::Runtime::new()
            .map_err(|e| format!("创建 tokio runtime 失败: {}", e))?;
        runtime.block_on(call_qoder_rpc_async(method_name, params))
    };

    match &result {
        Ok(value) => logger::log_info(&format!(
            "[Qoder OAuth] WebSocket 调用成功: method={}, elapsed={}ms, result_kind={}",
            method,
            started.elapsed().as_millis(),
            value_kind(value)
        )),
        Err(err) => logger::log_warn(&format!(
            "[Qoder OAuth] WebSocket 调用失败: method={}, elapsed={}ms, error={}",
            method,
            started.elapsed().as_millis(),
            err
        )),
...
fn value_to_bool(value: &Value) -> Option<bool> {
    ...
    if let Some(number) = value_to_i64(value) {
...
fn build_runtime_user_info_raw(account: &QoderAccount) -> Result<Value, String> {
    ...
    let map = user_info
        .as_object_mut()
        .ok_or_else(|| "Qoder userInfo 结构无效，无法执行运行时同步".to_string())?;

    let user_id = get_string_from_object(map, &["id", "uid"])
        .or_else(|| {
            account
                .user_id
                .as_deref()
        ...
        .ok_or_else(|| "Qoder 账号缺少 user id，无法执行运行时同步".to_string())?;
    map.insert("id".to_string(), Value::String(user_id.clone()));

    if get_string_from_object(map, &["email"]).is_none() {
        map.insert("email".to_string(), Value::String(account.email.clone()));
    }
    if get_string_from_object(map, &["name"]).is_none() {
    ...
    if get_string_from_object(map, &["token"]).is_none() {
        if let Some(token) = get_string_from_object(map, &["securityOauthToken"]) {
            map.insert("token".to_string(), Value::String(token));
        }
    }
    if get_string_from_object(map, &["token", "securityOauthToken"]).is_none() {
        return Err("Qoder 账号缺少 token，无法执行运行时同步".to_string());
    }
    if get_i64_from_object(map, &["status"]).is_none() {
        map.insert(
            "status".to_string(),
            Value::Number(serde_json::Number::from(AUTH_STATUS_AUTHORIZED)),
        );
    }
    if get_i64_from_object(map, &["whitelist"]).is_none() {
        let fallback_whitelist =
            get_i64_from_object(map, &["whitelistStatus"]).unwrap_or(WHITELIST_PASS);
        map.insert(
            "whitelist".to_string(),
            Value::Number(serde_json::Number::from(fallback_whitelist)),
        );
    }
    Ok(user_info)
}
...
fn build_sync_user_info_to_go_payload(user_info: &Value) -> Result<Value, String> {
    let source = user_info
        .as_object()
        .ok_or_else(|| "Qoder userInfo 结构无效，无法构造 syncUserInfo payload".to_string())?;

    let mut target = serde_json::Map::new();
    insert_i64_field(
        &mut target,
        "status",
        get_i64_from_object(source, &["status"]).unwrap_or(AUTH_STATUS_AUTHORIZED),
    );
    insert_i64_field(
        &mut target,
        "whitelistStatus",
        get_i64_from_object(source, &["whitelistStatus", "whitelist"]).unwrap_or(WHITELIST_PASS),
    );
    insert_string_field(
        &mut target,
        "name",
        get_string_from_object(source, &["name"]),
    );
    insert_string_field(
        &mut target,
        "aid",
        get_string_from_object(source, &["accountId", "aid", "id"]),
    );
    insert_string_field(
        &mut target,
        "uid",
        get_string_from_object(source, &["id", "uid"]),
    );
    insert_string_field(
        &mut target,
        "email",
        get_string_from_object(source, &["email"]),
    );
    insert_string_field(
        &mut target,
        "avatarUrl",
        get_string_from_object(source, &["avatarUrl", "avatar_url"]),
    );
    insert_string_field(
        &mut target,
        "orgId",
        get_string_from_object(source, &["orgId"]),
    );
    insert_string_field(
        &mut target,
        "orgName",
        get_string_from_object(source, &["orgName"]),
    );
    insert_string_field(
        &mut target,
        "yxUid",
        get_string_from_object(source, &["yxUid"]),
    );
    insert_string_field(
        &mut target,
        "staffId",
        get_string_from_object(source, &["staffId"]),
    );
    insert_string_field(
        &mut target,
        "securityOauthToken",
        get_string_from_object(source, &["token", "securityOauthToken"]),
    );
    insert_string_field(
        &mut target,
        "refreshToken",
        get_string_from_object(source, &["refreshToken"]),
    );
    if let Some(token_expire_time) = get_i64_from_object(source, &["expireTime", "tokenExpireTime"])
    {
        insert_i64_field(&mut target, "tokenExpireTime", token_expire_time);
    }
    if let Some(user_type) = get_i64_from_object(source, &["userType"]) {
        insert_i64_field(&mut target, "userType", user_type);
    }
    if let Some(agreed) = get_bool_from_object(source, &["privacyPolicyAgreed", "dataPolicyAgreed"])
    {
        target.insert("dataPolicyAgreed".to_string(), Value::Bool(agreed));
    }
    if let Some(modifiable) = get_bool_from_object(
        source,
        &["isPrivacyPolicyModifiable", "isDataPolicyModifiable"],
    ) {
        target.insert(
            "isDataPolicyModifiable".to_string(),
            Value::Bool(modifiable),
        );
    }
    if let Some(quota_exceeded) = get_bool_from_object(source, &["isQuotaExceeded"]) {
        target.insert("isQuotaExceeded".to_string(), Value::Bool(quota_exceeded));
    }
    insert_string_field(
        &mut target,
        "userSourceChannel",
        get_string_from_object(source, &["userSourceChannel"]),
    );

    Ok(json!({
        "syncEventType": QODER_SYNC_EVENT_LOGIN,
        "userInfo": Value::Object(target)
    }))
}
...
fn build_switch_account_payload(user_info: &Value) -> Option<Value> {
    ...
    let org_id = get_string_from_object(source, &["orgId"]);
    let mut payload = serde_json::Map::new();
    payload.insert("userId".to_string(), Value::String(user_id));
    if let Some(org_id) = org_id {
        payload.insert("orgId".to_string(), Value::String(org_id));
    }
...
fn sync_runtime_login_state_once(user_info: &Value) -> Result<(), String> {
    ...
    if let Some(switch_payload) = build_switch_account_payload(user_info) {
        call_qoder_rpc("user:switchAccount", switch_payload)?;
    }
    call_qoder_rpc(
        "sync.cached.auth",
        json!({
...
pub async fn sync_runtime_login_state(account: &QoderAccount) -> Result<(), String> {
    ...
    for attempt in 1..=RUNTIME_SYNC_MAX_ATTEMPTS {
        match sync_runtime_login_state_once(&user_info) {
            Ok(_) => {
                logger::log_info(&format!(
                    "[Qoder OAuth] 运行时登录态同步成功: account_id={}, attempt={}",
                    account.id, attempt
                ));
                return Ok(());
            }
            Err(err) => {
                ...
                if attempt < RUNTIME_SYNC_MAX_ATTEMPTS {
                    logger::log_warn(&format!(
                        "[Qoder OAuth] 运行时登录态同步失败，等待重试: account_id={}, attempt={}/{}, error={}",
                        account.id,
                        attempt,
                        RUNTIME_SYNC_MAX_ATTEMPTS,
                        err_text
                    ));
                    tokio::time::sleep(Duration::from_millis(RUNTIME_SYNC_RETRY_DELAY_MS)).await;
                }
            }
        }
    }
