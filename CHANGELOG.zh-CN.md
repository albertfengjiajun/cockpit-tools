...
- **Codex API 服务默认在官方客户端输入框下方显示账号池与聚合额度**：新安装及尚未保存过该设置的升级用户默认开启，重启对应 Codex 实例后即可查看账号池数量、5h 额度和周额度；已手动关闭的用户继续保持关闭。浮层会实时跟随窗口与输入区布局变化，不修改或挤压官方界面；该能力仅用于 Cockpit Tools API 服务，不会对普通 API Key 或自定义 Provider Gateway 启用。
